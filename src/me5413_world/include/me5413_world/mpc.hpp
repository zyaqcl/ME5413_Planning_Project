...
class MPC
{
 public:
  MPC() {};
  MPC(double dt, double max, double min, double Kp, double Kd, double Ki);
  ~MPC() {};
...
MPC::MPC(double dt, double max, double min, double Kp, double Kd, double Ki) :
  dt_(dt),
  max_(max),
  min_(min),
  Kp_(Kp),
...
void MPC::updateSettings(const double Kp, const double Kd, const double Ki)
{
  this->Kp_ = Kp;
  this->Kd_ = Kd;
  this->Ki_ = Ki;
...
double MPC::calculate(const double setpoint, const double pv)
{
  // Calculate error
